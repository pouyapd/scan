...
We only have made some changes to integrate our library*/

//! Model builder for SCAN's XML specification format.

use crate::parser::{OmgType, OmgTypes, Parser};
use anyhow::anyhow;
...
//Adding class using scxml_lib
use scxml_lib::build_parser::{Executable, Param, Scxml};
use scxml_lib::build_parser::send::Target;
...
impl ModelBuilder {
    ...
    fn prebuild_fsms(
        ...
        for data in &fmt.get_datamodel() {
            types.insert(data.get_id().to_owned(), data.get_omg_type().as_str().to_owned());
        }
        for (_, state) in fmt.get_states().iter_mut() {
            for exec in state.get_on_entry().iter_mut() {
                self.prebuild_exec(pg_id, exec, &types, interner)?;
            }
            for transition in state.get_transitions().iter_mut() {
                if let Some(ref event) = transition.get_event() {
                ...
                for exec in transition.get_effects().iter_mut() {
                    self.prebuild_exec(pg_id, exec, &types, interner)?;
                }
            }
            for exec in state.get_on_exit().iter_mut() {
                self.prebuild_exec(pg_id, exec, &types, interner)?;
            }
        }
        Ok(())
    ...
    fn prebuild_exec(
        ...
        match executable {
            ...
            Executable::Send(send) => {
                let event_index = self.event_index(&send.get_event());
                let builder = self.events.get_mut(event_index).expect("index must exist");
                builder.senders.insert(pg_id);
                for mut param in send.get_params() {
                    let param_type = param.get_omg_type().to_owned();
                    let param_type = param_type
                        .or_else(|| self.infer_type(&param.get_expr(), types, interner).ok())
                        .ok_or(anyhow!("missing type annotation for param {}", param.get_name()))?;
                    // Update omg_type value so that it contains its type for sure
                    param.set_omg_type(Some(param_type.to_owned()));
                    let builder = self.events.get_mut(event_index).expect("index must exist");
                    let prev_type = builder
                        .params
                        .insert(param.get_name().to_owned(), param_type.to_owned());
            ...
            Executable::If(r#if) => {
                ...
                for (_, executables) in r#if.get_r_elif() {
                    for mut executable in executables {
                        self.prebuild_exec(pg_id, &mut executable, types, interner)?;
                    }
                }
                for mut executable in r#if.get_r_else() {
                    self.prebuild_exec(pg_id, &mut executable, types, interner)?;
                }
                Ok(())
            }
    ...
    fn build_fsm(&mut self, scxml: &Scxml, interner: &mut Interner) -> anyhow::Result<()> {
        trace!(target: "build", "build fsm {}", scxml.get_id());
        // Initialize fsm.
        let pg_builder = self
            .fsm_builders
            .get(&scxml.get_id())
            .unwrap_or_else(|| panic!("builder for {} must already exist", scxml.get_id()));
        ...
        for data in scxml.get_datamodel().iter() {
            let scan_type = self
                .types
                .get(data.get_omg_type().as_str())
            ...
            vars.insert(data.get_id().to_owned(), (var, data.get_omg_type().to_owned()));
            // Initialize variable with `expr`, if any, by adding it as effect of `initialize` action.
            if let Some(ref expr) = data.get_expression() {
        ...
        states.insert(scxml.get_initial().to_owned(), initial_state);
        ...
        for (state_id, state) in scxml.get_states().iter() {
            ...
            for executable in state.get_on_entry().iter() {
            ...
            for transition in state.get_transitions().iter() {
                ...
                let target_loc = *states
                    .entry(transition.get_target().to_owned())
                ...
                let exec_origin;
                let exec_params;
                if let Some(event_name) = transition.get_event().as_ref() {
                ...
                for exec in state.get_on_exit().iter().chain(transition.get_effects().iter()) {
    ...
    fn add_executable(
        ...
        match executable {
            ...
            Executable::Send(send) => {
                ...
                let mut loc = loc;
                if let Some(delay) = &send.get_delay() {
                ...
                if let Some(target) = send.get_target() {
                    ...
                    for target in targets {
                        ...
                        next_loc = self.add_executable(
                            &Executable::Send(scxml_lib::build_parser::Send::new(
                                send.get_event().to_owned(), 
                                target_name.map(Target::Id), 
                                *&send.get_delay(), 
                                send.get_params().to_owned())
            ...
            Executable::If(r#if) => {
                ...
                for (cond, execs) in r#if.get_r_elif() {
    ...
    fn send_param(
        ...
        let scan_type = self
            .types
            .get(param.get_omg_type().as_ref().expect("type name annotation"))
            .cloned()
        ...
        let param_chn = *self
            .parameters
            .entry((pg_id, target_id, event_idx, param.get_name().to_owned()))
