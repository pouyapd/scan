...
All the parts that concern the reading of .xml files have not been modified.
Instead, we have modified the part that concerns the reading of .scxml files 
which are passed to our library (scxml_lib) and converted into an SCXML class.*/
...
impl Parser {
    ...
    fn parse_file(&mut self, path: &Path) -> anyhow::Result<()> {
        if path.is_dir() {
            ...
            match ext {
                "scxml" => {
                     //This part has been removed as it is no longer needed using our library
                }
                "xml" => {
                    ...
                    match tag_name {
                        ...
                        TAG_PROCESS if stack.last().is_some_and(|e| *e == ConvinceTag::ProcessList) => {
                           ...
                           /*if self.process_list.contains_key(&process_id) {
                            ...
                            /*The id is read from the model.xml file 
                            but the fsm.scxml file is handled by our library and the resulting Scxml class is inserted into process_list.*/
                            self.process_list.insert(process_id, scxml_lib::build(path.clone(),&mut self.interner));
                        }
                        _ => {
